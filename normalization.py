import pandas as pd
writer = pd.ExcelWriter('document/Normalized.xlsx')
items = ['谷物类','淀粉类','坚果及种子类','水果类','蔬菜类','藻类','菇类','豆类','糕饼点心类','肉类','鱼贝类','蛋类','乳品类','油脂类','糖类','饮料类','调味料及香辛料类','加工调理食品及其他类']

def minMaxNormalization(df):
    return (df-df.min())/(df.max()-df.min())

data = pd.read_excel('document/STaiwan_filled0.xlsx')
data = data.fillna(value=0.0)
for i in items:
    to = data[data['食品分类']==i]
    to_= data[data['食品分类']==i]
    to.drop(['index','样品编号','食品分类','样品名称','内容物描述','废弃率(%)','P/M/S'],axis=1,inplace=True)
    ...
    normalized.fillna(value=0.0,inplace=True)
