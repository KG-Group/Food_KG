...
def upload(Food, Dish, graph,matcher):
    TypeList = ["谷物类","淀粉类","坚果及种子类","水果类","蔬菜类","藻类","菇类","豆类","肉类","鱼贝类","蛋类","乳品类","油脂类","饮料类","调味料及香辛料类","糕饼点心类","加工调理食品及其他类","糖类"]
    DNode = matcher.match("Dish",name = Dish).first()
    FNode = None
    for Type in TypeList:
        Node = matcher.match(Type, name = Food).first()
        if(Node):
            FNode = Node
    Edge = Relationship(DNode,"包含",FNode)
    graph.create(Edge)
    graph.push(Edge)
...
for i in range(0,121):
    ...
    for item in nowRow[4:]:
        ...
        for Food in FoodList:
            if(name == Food["样品名称"]):
                upload(Food["样品名称"],nowRow[0],graph,matcher)
                best = 1
                good = 1
                break
        if(best == 0):
        ...
        if(good == 0):
            ...
            upload(bestkey["样品名称"],nowRow[0],graph,matcher)
